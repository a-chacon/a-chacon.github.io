...
Si te ha interesado el tema, te invito a buscar más información en la [documentación](https://docs.ruby-lang.org/en/master/syntax/pattern_matching_rdoc.html). Existen otros elementos interesantes de la coincidencia de patrones, como el uso del operador pin (^) y *Guard clauses* (`if` y `unless`).
